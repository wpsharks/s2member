	...
	class c_ws_plugin__s2member_menu_page_down_ops
	{
		public function __construct()
		{
			...
			if(apply_filters("ws_plugin__s2member_during_down_ops_page_during_left_sections_display_amazon_s3", TRUE, get_defined_vars()))
			{
				...
				//260916.2327 Include commercial AWS regions launched before the 2019 regional-endpoint cutoff that remain compatible with s2Member's legacy S3 endpoint handling.
				echo '<option value="us-east-2"'.(($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["amazon_s3_files_bucket_region"] === 'us-east-2') ? ' selected="selected"' : '').'>us-east-2</option>'."\n";
				...
				echo '<option value="ca-central-1"'.(($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["amazon_s3_files_bucket_region"] === 'ca-central-1') ? ' selected="selected"' : '').'>ca-central-1</option>'."\n";
				...
				echo '<option value="eu-west-3"'.(($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["amazon_s3_files_bucket_region"] === 'eu-west-3') ? ' selected="selected"' : '').'>eu-west-3</option>'."\n";
				...
				echo '<option value="eu-north-1"'.(($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["amazon_s3_files_bucket_region"] === 'eu-north-1') ? ' selected="selected"' : '').'>eu-north-1</option>'."\n";
				echo '<option value="ap-south-1"'.(($GLOBALS["WS_PLUGIN__"]["s2member"]["o"]["amazon_s3_files_bucket_region"] === 'ap-south-1') ? ' selected="selected"' : '').'>ap-south-1</option>'."\n";
			...
			if(apply_filters("ws_plugin__s2member_during_down_ops_page_during_left_sections_display_shortcode_attrs", TRUE, get_defined_vars()))
			{
				...
				//260805 Document the exact trusted player script paths accepted from shortcode content.
				echo '<li><code>player_path="/jwplayer/jwplayer.js"</code> Required. This must exactly match a trusted JW Player JavaScript path allowed by s2Member. The default allowed path is <code>/jwplayer/jwplayer.js</code> (you should upload the <a href="http://www.s2member.com/r/jw-player-download/" target="_blank" rel="external">/jwplayer</a> folder to the root of your web directory). Developers may allow custom exact paths through the <code>ws_plugin__s2member_sc_get_stream_player_paths</code> PHP filter; the first allowed path is used as the fallback.</li>'."\n";
				//260805 Document the validated resolution identifiers used in filenames and player labels.
				echo '<li><code>player_resolutions=""</code> Optional (requires s2Member Pro). This is a comma-delimited list of all available resolution options (should you decide to offer more than just one file download at a single resolution). Each resolution identifier may contain letters, numbers, underscores, and hyphens only; e.g., <code>360p,720p-HD,1080p</code>. Please review the full list of Shortcode Attributes (i.e., click the "Shortcode Attributes (Explained)" tab) in <a href="http://www.s2member.com/kb/jwplayer-s2stream-shortcodes/#using-s2stream-shortcodes" target="_blank" rel="external">this KB article</a> for further details, requirements, and an example of use.</li>'."\n";
				//260805 Clarify that only the shortcode attributes declared by s2Member are accepted and structured values are data-only.
				echo '<li><code>player_{setting}=""</code> Optional. Supported player configuration attributes are prefixed with <code>player_</code>; this notation does not create arbitrary new shortcode attributes. For JW Player v7, see <a href="http://www.s2member.com/r/jw-player-config-options/" target="_blank" rel="external">this article please</a>. Examples: <code>player_width="480"</code>, <code>player_height="270"</code>, <code>player_title="My Video"</code>, <code>player_description="A video about something."</code>, <code>player_image="http://www.example.com/wp-content/uploads/video-preview.jpg"</code>, <code>player_mediaid="ei0wsx23"</code>, <code>player_autostart="true"</code>, <code>player_key="my-license-key"</code>, <code>player_tracks="{kind:\'captions\',file:\'/assets/captions-en.vtt\',label:\'English\'}"</code> (<em>With <a href="http://www.s2member.com/r/jw-player-video-captions/" target="_blank" rel="external">Captions</a>, you can exclude the square array brackets to avoid Shortcode parsing issues. s2Member will automatically wrap your Caption objects with square array brackets. <code>player_tracks</code> and the legacy <code>player_captions</code> attribute accept data-only objects/arrays; invalid values are ignored.</em>). Please note that "Advanced Options Blocks" listed on <a href="http://www.s2member.com/r/jw-player-config-options/" target="_blank" rel="external">this page</a> are NOT supported here. For those, please use: <code>player_option_blocks=""</code> (see below).</li>'."\n";
				//260805 Define Advanced Option Blocks as configuration data, not executable JavaScript supplied through post content.
				echo '<li><code>player_option_blocks=""</code> Optional. Data-only "Advanced Option Blocks" supported by your audio/video player. For JW Player v7, see <a href="http://www.s2member.com/r/jw-player-config-options/" target="_blank" rel="external">this article please</a>. Here are some examples: <code>player_option_blocks="sharing:{}"</code>, <code>player_option_blocks="sharing:{}, logo: {file: \'/logo.png\', link: \'http://example.com\'}"</code>. Or: <code>player_option_blocks="c2hhcmluZzoge30="</code> (base64 encoded version of <code>sharing:{}</code>). Advanced Option Blocks can be defined in plain text or with a <a href="http://s2member.com/r/base64-encoding/" target="_blank" rel="external">base64 encoded string</a>, and may contain objects, arrays, strings, numbers, booleans, and null values. Invalid blocks are ignored. Player/plugin script loaders must be configured through trusted PHP instead of shortcode content. If you have trouble defining object properties inside a Shortcode Attribute, please use <a href="http://s2member.com/r/base64-encoding/" target="_blank" rel="external">this tool</a> to base64 encode your Advanced Option Blocks, so that you end up with a string that\'s compatible with Shortcode Attributes.</li>'."\n";
