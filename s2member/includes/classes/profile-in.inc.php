			...
			if(!empty($_GET['s2member_profile'])) // Requesting Profile?
			{
				...
				if(($user = (is_user_logged_in()) ? wp_get_current_user() : FALSE) && ($user_id = $user->ID))
				{
					...
					if($GLOBALS['WS_PLUGIN__']['s2member']['o']['custom_reg_opt_in'] && c_ws_plugin__s2member_list_servers::list_servers_integrated())
					{
						...
						echo '<tr>'."\n";
						echo '<td>'."\n";
						echo '<label for="ws-plugin--s2member-profile-field-opt-in">'."\n";
						echo '<input type="checkbox" name="ws_plugin__s2member_profile_field_opt_in" id="ws-plugin--s2member-profile-field-opt-in" class="ws-plugin--s2member-profile-field" value="1" checked="checked" tabindex="'.esc_attr(($tabindex = $tabindex + 10)).'" />'."\n";
