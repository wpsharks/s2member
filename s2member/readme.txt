...
* (s2Member/s2Member Pro) **Logging Functionality** Additional logging routines that will track all HTTP communication within WordPress® are now possible (these are quite extensive). See: `Dashboard -› s2Member® -› Log Files (Debug) -› Logging Configuration` for further details. This extensive logging is disabled by default; it must be enabled by a site owner. Not recommended once a site goes live.
